...
export function CustomersClient({ initialUsers }: { initialUsers: any[] }) {
  ...
  const handleSuspendUser = async (u: any) => {
    ...
    const isConfirmed = await confirm({
      ...
      description: isSuspended
        ...
        : "They will not be able to log in or place orders until unsuspended.",
    ...
    if (isConfirmed) {
      ...
      if (res.success) {
        setUsers(
          users.map((user) =>
            user._id === u._id ? { ...user, isSuspended: !isSuspended } : user,
          ),
        );
  ...
  const handleChangePasswordSubmit = async () => {
    ...
    const res = await adminChangePasswordAction(
      passwordChangeUser._id,
      newPassword,
    );
    if (res.success) {
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6">
        ...
        <div className="flex items-center gap-3 bg-white border-none px-4 py-2">
          <span className="text-[11px] uppercase tracking-wider font-bold text-text-muted">
            Total Users
          </span>
          <span className="w-px h-4 bg- mx-1" />
      ...
      <div className="bg-white border-none p-4 flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
        <div className="flex flex-1 items-center gap-3 w-full">
          ...
          >
            <SelectTrigger className="w-[180px] h-12 rounded-2xl text-[11px] uppercase tracking-wider font-bold focus:ring-0 bg-white text-[#222222]">
        ...
        {(search || roleFilter !== "all") && (
          <button
            ...
            className="h-12 px-4 text-[11px] uppercase tracking-wider font-bold text-text-muted hover:text-red-600 transition-colors flex items-center justify-center gap-2 border-none hover: hover:bg-red-50"
          >
      ...
      <div className="bg-white border-none overflow-x-auto">
        <table className="w-full text-left ">
          <thead>
            <tr className=" bg-dasadeep/20">
              <th className="py-4 px-6 text-[11px] font-bold uppercase tracking-wider text-text-muted w-[250px]">
                User Profile
              </th>
              <th className="py-4 px-6 text-[11px] font-bold uppercase tracking-wider text-text-muted">
                Role
              </th>
              <th className="py-4 px-6 text-[11px] font-bold uppercase tracking-wider text-text-muted">
                Joined Date
              </th>
              <th className="py-4 px-6 text-[11px] font-bold uppercase tracking-wider text-text-muted text-center">
                Orders
              </th>
              <th className="py-4 px-6 text-[11px] font-bold uppercase tracking-wider text-text-muted text-center w-24">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-">
            {users.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-20 text-center">
                  <div className="flex flex-col items-center justify-center text-text-muted">
                    <div className="bg-dasadeep/20 p-4 rounded-full mb-4">
                      <Search className="h-6 w-6 opacity-50" />
                    </div>
                    <p className="text-[13px] font-bold uppercase tracking-wider text-[#222222]">
                      No users found
                    </p>
              ...
              users.map((u) => (
                <tr
                  key={u._id}
                  className="group hover:bg-dasadeep/10 transition-colors"
                >
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        alt={u.name}
                        className="h-10 w-10 object-cover border-none "
                      />
                  ...
                  <td className="py-4 px-6">
                    <div className="flex flex-col items-start gap-1">
                      <span
                        className={`inline-flex items-center justify-center px-2.5 py-1 text-[9px] uppercase tracking-widest font-bold border-none ${getRoleBadgeStyle(u.role)}`}
                      >
                        {u.role}
                      </span>
                      {u.isSuspended && (
                        <span className="inline-flex items-center justify-center px-2.5 py-1 text-[9px] uppercase tracking-widest font-bold border-none bg-red-50 text-red-700 mt-1">
                          Suspended
                        </span>
                      )}
                    </div>
                  ...
                  <td className="py-4 px-6 text-center">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="h-8 w-8 inline-flex items-center justify-center text-text-muted hover:text-[#222222] hover:bg-dasadeep/50 transition-colors border-none hover:">
                          <MoreVertical className="h-4 w-4" strokeWidth={1.5} />
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent
                        align="end"
                        className="rounded-2xl shadow-sm w-48"
                      >
                        <DropdownMenuLabel className="text-[10px] uppercase tracking-wider text-text-muted font-bold px-3 py-2">
                          Actions
                        </DropdownMenuLabel>
                        <DropdownMenuItem
                          onClick={() => handleViewUser(u)}
                          className="text-[11px] uppercase tracking-wider font-bold text-[#222222] cursor-pointer rounded-2xl focus:bg-dasadeep/50 py-2.5 px-3"
                        >
                        ...
                        {user?.role === "admin" && (
                          <>
                            ...
                            <DropdownMenuItem
                              ...
                              className="text-[11px] uppercase tracking-wider font-bold text-[#222222] cursor-pointer rounded-2xl focus:bg-dasadeep/50 py-2.5 px-3"
                            >
                              <Lock className="mr-2 h-3.5 w-3.5" /> Change
                              Password
                            </DropdownMenuItem>
                            <DropdownMenuSeparator className="bg-" />
                            <DropdownMenuItem
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent
          ...
          className="max-w-6xl w-[95vw] h-[85vh] flex flex-col p-0 gap-0 overflow-hidden bg-[#fdfbf7] rounded-2xl border-none "
        >
          {selectedUser && (
            <>
              {/* Modal Header */}
              <div className="p-8 bg-white relative">
                <button
                  ...
                  className="absolute top-8 right-8 w-8 h-8 flex items-center justify-center border-none text-text-muted hover:text-[#222222] hover:bg-dasadeep/50 transition-colors"
                >
                  <X className="w-4 h-4" strokeWidth={1.5} />
                </button>
                <div className="flex items-center gap-6">
                  <img
                    ...
                    className="w-20 h-20 border-none object-cover"
                  />
                  <div className="space-y-2">
                    <DialogTitle className="text-[20px] uppercase tracking-widest font-bold text-[#222222]">
                      {selectedUser.name}
                    </DialogTitle>
                    <DialogDescription className="text-[12px] text-text-muted tracking-wider">
                      {selectedUser.email}
                    </DialogDescription>
                    <div className="flex flex-wrap items-center gap-4 mt-2">
                      <span
                        className={`inline-flex items-center justify-center px-2.5 py-1 text-[9px] uppercase tracking-widest font-bold border-none ${getRoleBadgeStyle(selectedUser.role)}`}
                      >
                        {selectedUser.role}
                      </span>
                      {selectedUser.isSuspended && (
                        <span className="inline-flex items-center justify-center px-2.5 py-1 text-[9px] uppercase tracking-widest font-bold border-none bg-red-50 text-red-700 ">
                          Suspended
                        </span>
                      )}
                      {selectedUser.createdAt && (
                        <>
                          <span className="w-px h-4 bg- mx-1 hidden sm:block" />
                  ...
                  <TabsContent value="profile" className="space-y-8">
                    {user?.role === "admin" && (
                      <div className="bg-white border-none p-6 space-y-6">
                        ...
                        <div className="bg-orange-50 border-none p-4 text-[11px] text-orange-800 tracking-wider">
                          <strong>WARNING:</strong> Changing a user's role
                          grants them different permissions. Admins have full
                          access.
                        </div>
                        ...
                        <div className="max-w-xs space-y-2">
                          ...
                          >
                            <SelectTrigger className="h-12 rounded-2xl text-[11px] uppercase tracking-wider font-bold focus:ring-0 bg-dasadeep/20 text-[#222222]">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="rounded-2xl ">
                              <SelectItem
                    ...
                    <div className="bg-white border-none p-6 space-y-6">
                      <h3 className="text-[12px] uppercase tracking-widest font-bold text-[#222222] pb-4">
                        Contact Information
                      </h3>
                  ...
                  <TabsContent value="orders">
                    <div className="bg-white border-none overflow-hidden">
                      <div className="p-6 flex items-center justify-between">
                        ...
                        {userOrders.length > 0 && (
                          <span className="text-[10px] font-bold uppercase tracking-widest text-text-muted border-none px-2 py-1">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left ">
                          <thead>
                            <tr className=" bg-dasadeep/20">
                              <th className="py-3 px-6 text-[10px] font-bold uppercase tracking-widest text-text-muted">
                                Order ID
                              </th>
                              <th className="py-3 px-6 text-[10px] font-bold uppercase tracking-widest text-text-muted">
                                Date
                              ...
                              <th className="py-3 px-6 text-[10px] font-bold uppercase tracking-widest text-text-muted text-right">
                                Total
                              </th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-">
                            {ordersLoading ? (
                              <tr>
                                <td
                                  colSpan={5}
                                  className="py-12 text-center text-[11px] text-text-muted uppercase tracking-wider font-bold"
                                >
                                  Loading orders...
                                </td>
                              </tr>
                              ...
                              userOrders.map((order) => (
                                <tr
                                  key={order._id}
                                  className="hover:bg-dasadeep/10 transition-colors"
                                >
                                  ...
                                  <td className="py-4 px-6">
                                    <span
                                      className={`inline-flex items-center justify-center px-2 py-1 text-[9px] uppercase tracking-widest font-bold border-none ${getOrderStatusColor(order.orderStatus)}`}
                                    >
                                      {order.orderStatus}
                                    </span>
                                  </td>
                                  <td className="py-4 px-6">
                                    <div className="flex items-center gap-2">
                                      <span
                                        className={`w-2 h-2 rounded-2xl ${order.paymentStatus === "paid" ? "bg-primary" : order.paymentStatus === "pending" ? "bg-orange-500" : "bg-red-500"}`}
                                      />
                                      <span className="text-[11px] uppercase tracking-wider font-bold text-[#222222]">
                                        {order.paymentStatus}
                                      </span>
                                    </div>
                                  </td>
              ...
              <DialogFooter className="p-6 bg-white">
                <button
                  onClick={() => setIsDetailOpen(false)}
                  className="bg-white border-none text-[#222222] px-6 py-3 text-[11px] uppercase tracking-wider font-bold hover:bg-dasadeep/50 transition-colors"
                >
                  Close Details
                </button>
              </DialogFooter>
      ...
      >
        <DialogContent className="rounded-2xl border-none bg-white sm:max-w-md">
          <DialogTitle className="text-[14px] uppercase tracking-widest font-bold text-[#222222] pb-4">
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-12 rounded-2xl focus-visible:ring-0 focus-visible:"
              />
            </div>
          </div>
          <DialogFooter>
            <button
              ...
              className="px-6 py-3 border-none text-[10px] uppercase tracking-widest font-bold text-text-muted hover:text-black hover:bg-dasadeep/50 transition-colors"
            >
              Cancel
            </button>
            <button
