...
export function OrdersClient({
  ...
  const handleBatchStatusUpdate = async (newStatus: string) => {
    ...
    const res = await batchUpdateOrderStatusAction(
  ...
  const isFilterActive =
    statusFilter.length > 0 || dateFilter !== "all" || searchTerm !== "";
    ...
    >
      ...
      <div className="bg-white border-none p-4 flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-64 rounded-2xl shadow-sm p-2"
            >
              <DropdownMenuLabel className="text-[10px] uppercase tracking-wider text-text-muted font-bold px-2 py-2">
                Filter by Status
              </DropdownMenuLabel>
              ...
              {Object.keys(STATUS_CONFIG).map((key) => (
                <DropdownMenuCheckboxItem
                  ...
                  className="text-[12px] rounded-2xl focus:bg-dasadeep/50 focus:text-[#222222] py-2 cursor-pointer"
                >
              ...
              {statusFilter.length > 0 && (
                <>
                  ...
                  <DropdownMenuItem
                    ...
                    className="justify-center text-center text-[10px] uppercase tracking-wider font-bold text-[#222222] hover:bg-dasadeep/50 rounded-2xl cursor-pointer py-2"
                  >
                    CLEAR STATUS
                  </DropdownMenuItem>
                </>
      ...
      <div className="bg-white border-none ">
        {selectedOrders.size > 0 && (
          <div className="bg-dasadeep/20 px-4 py-3 flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-4">
              ...
              <span className="w-px h-4 bg-" />
              {(user?.role === "admin" || user?.role === "manager") && (
                <Select
                  onValueChange={handleBatchStatusUpdate}
                  disabled={batchActionLoading}
                >
                  <SelectTrigger className="h-8 bg-white text-[11px] uppercase tracking-wider font-bold w-48 focus:ring-0">
            ...
            <button
              ...
              className="text-[11px] uppercase tracking-wider font-bold text-text-muted hover:text-[#222222]"
            >
              CANCEL
            </button>
          </div>
        ...
        {initialOrders.length === 0 ? (
          <div className="text-center py-20">
            <Package className="w-12 h-12 text- mx-auto mb-4" />
            <h3 className="text-[14px] font-bold text-[#222222] uppercase tracking-widest mb-1">
              No Orders Found
            </h3>
            ...
            {isFilterActive && (
              <button
                onClick={clearFilters}
                className="mt-6 border-none bg-white text-[#222222] px-6 py-2.5 text-[11px] uppercase tracking-wider font-bold hover:bg-dasadeep/50 transition-colors inline-block"
              >
                CLEAR ALL FILTERS
              </button>
            )}
            ...
            <table className="w-full text-left ">
              <thead>
                <tr className=" bg-dasadeep/10">
                  ...
                  <th className="py-4 px-4 font-bold text-[10px] uppercase tracking-widest text-text-muted">
                    Order ID & Date
                  </th>
                  <th className="py-4 px-4 font-bold text-[10px] uppercase tracking-widest text-text-muted">
                    Customer
                  ...
                  <th className="py-4 px-4 font-bold text-[10px] uppercase tracking-widest text-text-muted text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody>
                {initialOrders.map((order) => {
                  const StatusIcon =
                    STATUS_CONFIG[order.orderStatus]?.icon || AlertCircle;
                  return (
                    <tr
                      key={order._id}
                      className={` hover:bg-dasadeep/10 transition-colors group cursor-pointer ${selectedOrders.has(order.paymentReference) ? "bg-dasadeep/20" : ""}`}
                      onClick={() => {
                    ...
                    >
                      ...
                      >
                        {user?.role === "admin" || user?.role === "manager" ? (
                          ...
                          <span
                            className={`inline-flex items-center justify-center px-2 py-1 text-[9px] uppercase tracking-widest font-bold border-none ${STATUS_CONFIG[order.orderStatus]?.color || ""}`}
                          >
                      ...
                      >
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button className="p-2 hover:bg-dasadeep/50 transition-colors text-text-muted hover:text-[#222222]">
                              <MoreVertical className="h-4 w-4" />
                            </button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent
                            align="end"
                            className="w-48 rounded-2xl shadow-sm p-1"
                          >
                            <DropdownMenuItem
                              className="text-[11px] uppercase tracking-wider font-bold text-[#222222] rounded-2xl focus:bg-dasadeep/50 cursor-pointer"
                              onClick={() => {
                                setSelectedOrder(order);
                                setIsSheetOpen(true);
                              }}
                            ...
                            <DropdownMenuSeparator className="bg- my-1" />
                            {user?.role === "admin" && (
                              <DropdownMenuItem
                                className="text-[11px] uppercase tracking-wider font-bold text-red-600 rounded-2xl focus:bg-red-50 focus:text-red-700 cursor-pointer"
                                onClick={() => handleDelete(order._id)}
                              >
                                <XCircle className="mr-2 h-3.5 w-3.5" /> Delete
                                Order
                              </DropdownMenuItem>
                            )}
                          </DropdownMenuContent>
        ...
        {pagination.totalPages > 1 && (
          <div className=" p-4 flex items-center justify-between bg-dasadeep/10">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center border-none bg-white text-[#222222] hover:bg-dasadeep/50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                  ...
                  if (
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 flex items-center justify-center border-none text-[11px] font-bold transition-colors ${
                          page === p
                            ? "bg-[#222222] ] text-white"
                            : "bg-white text-[#222222] hover:bg-dasadeep/50"
                        }`}
                      >
                        {p}
                      </button>
      ...
      <Dialog open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <DialogContent
          ...
          className="max-w-4xl p-0 gap-0 bg-white border-none rounded-2xl shadow-2xl h-[90vh] md:h-[85vh] overflow-hidden flex flex-col my-4 mx-4 md:mx-auto"
        >
          {selectedOrder && (
