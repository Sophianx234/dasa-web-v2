...
export default async function ProductsTab(props: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
 ...
 className={`pb-3 text-[12px] uppercase tracking-wider font-bold transition-all ${
 currentTab === 'products' 
 ? ' text-black' 
 ...
 <>
 {/* Search & Filter Bar */}
 <Suspense fallback={<div className="bg-white border-none p-5 h-20 animate-pulse" />}>
 <FiltersSection />
 </Suspense>
...
async function ProductsTableSection({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
 ...
 return (
 <div className="text-center p-16 bg-white border-none mt-8 flex flex-col items-center justify-center gap-4">
